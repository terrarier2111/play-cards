            ...
            DiagnosticItem::Warn(str, span) => {
                if !span.is_none() {
                    DiagnosticBuilder::build_span_string(span)
                        ...
                        + "\n"
                        + &" ".repeat(span.start)
                        + &str.to_string().yellow().to_string()
                        + "\n"
                } else {
                    str.to_string().yellow().to_string() + "\n"
                }
...
macro_rules! diagnostic_builder {
    ($error:literal) => {
        Err(anyhow::Error::new(
            crate::error::DiagnosticBuilder::from_err($error.to_string()),
        ))
    };
    ($error:expr) => {
        Err(anyhow::Error::new(
            crate::error::DiagnosticBuilder::from_err($error),
        ))
    };
    ($error:literal, $sp:expr) => {
        Err(anyhow::Error::new(
            crate::error::DiagnosticBuilder::from_err_with_span(
                $error.to_string(),
    ...
    ($error:expr, $sp:expr) => {
        Err(anyhow::Error::new(
            crate::error::DiagnosticBuilder::from_err_with_span($error, Span::single_token($sp)),
        ))
    };
}
...
macro_rules! diagnostic_builder_spanned {
    ($error:literal, $sp:expr) => {
        Err(anyhow::Error::new(
            crate::error::DiagnosticBuilder::from_err_with_span($error.to_string(), $sp),
        ))
    };
    ($error:expr, $sp:expr) => {
        Err(anyhow::Error::new(
            crate::error::DiagnosticBuilder::from_err_with_span($error, $sp),
        ))
    };
}
